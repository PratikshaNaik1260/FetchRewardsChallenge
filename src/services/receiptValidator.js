...
const validateReceipt = (receipt) => {
    ...
    if (typeof receipt.purchaseDate !== 'string' || receipt.purchaseDate.trim() === '' || !isValidDate(receipt.purchaseDate)) {
        errors.push('Invalid purchase date. Expected format is yyyy-mm-dd and a real date.');
    }

    if (typeof receipt.purchaseTime !== 'string' || receipt.purchaseTime.trim() === '' || !receipt.purchaseTime.match(/^([01]\d|2[0-3]):([0-5]\d)$/)) {
        errors.push('Invalid purchase time format. Expected format is hh:mm.');
    }

    if (typeof receipt.total!== 'string' || receipt.total.trim() === '' || !isNumeric(receipt.total)) {
    ...
    if (!Array.isArray(receipt.items) || receipt.items.length === 0) {
        ...
        if (!isNumeric(receipt.total) || !isNumeric(totalFromReceipt)) {
